...
`kube-bench` is a tool that automates the process of checking Kubernetes clusters for security vulnerabilities. See [here](https://github.com/aquasecurity/kube-bench)
...
```shell
KUBECONFIG=/etc/kubernetes/kubelet.conf ./kube-bench run -D cfg/ --config cfg/config.yaml > /var/www/html/index.html
...
According to the `kube-bench` suggestions, Kubelet configuration should be updated.
...
To fix the flagged issue on ETCD,
...
spec:
  ...
  - command:
    - kube-apiserver
    - --profiling=false # 1.2.21 Ensure that the --profiling argument is set to false
    - --insecure-port=0 # 1.2.19 Ensure that the --insecure-port argument is set to 0
    - --audit-log-path=/var/log/apiserver/audit.log # 1.2.22 Ensure that the --audit-log-path argument is set
    - --audit-log-maxage=30 # 1.2.23 Ensure that the --audit-log-maxage argument is set to 30 or as appropriate
    - --audit-log-maxbackup=10 # 1.2.24 Ensure that the --audit-log-maxbackup argument is set to 10 or as appropriate
    - --audit-log-maxsize=100 # 1.2.25 Ensure that the --audit-log-maxsize argument is set to 100 or as appropriate
    - --enable-admission-plugins=NodeRestriction,PodSecurityPolicy # 1.2.16 Ensure that the admission control plugin PodSecurityPolicy is set
    # REDACTED
...
`kube-bench` output file from the api-server, before the fixes are applied
